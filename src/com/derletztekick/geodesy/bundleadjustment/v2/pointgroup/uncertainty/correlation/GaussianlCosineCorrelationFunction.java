...
public class GaussianlCosineCorrelationFunction extends CorrelationFunction {
	public GaussianlCosineCorrelationFunction(double sigma, double maxDT, double a, double b, double c) {
	...
	double getCovarianceValue(double dT) {
		...
		return s2 * a * Math.exp(-b * dT*dT) * Math.cos(c * dT*dT);
	}
	
	@Override
	public int getType() {
		return CorrelationFunction.GAUSSIAN_CORR_FUN;
	}
}